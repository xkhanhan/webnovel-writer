...
## v6.4.0 - 新增独立润色 agent 和 /webnovel-polish skill

发版范围：`v6.3.3..v6.4.0`。

### 给作者看的变化

- 新增 `/webnovel-polish` 命令：独立润色章节正文，去除 AI 痕迹，按文风参考让句子更像人类作者写的小说。不影响故事结构、设定和情节走向。
- 支持 `--dry-run` 模式：只看润色建议不写文件。
- 润色后自动输出摘要（删除/替换/调整/增加各多少处）。

### 是否需要改旧项目

不需要。已有书项目可直接使用 `/webnovel-polish`。

### 给维护者

- 新增 `agents/polisher.md`：纯文风润色 agent，只改"怎么写"不改"写了什么"。
- 新增 `skills/webnovel-polish/SKILL.md`：独立 skill，解析项目根和题材后调用 polisher agent。

## v6.3.3 - Step 1.1 新增 writer 输入日志

发版范围：`v6.3.2..v6.3.3`。

### 给作者看的变化

- 写章流程新增 Step 1.1：context-agent 返回任务书后，主流程自动把任务书原文写入 `.webnovel/tmp/webnovel-writer_input_ch{章号}.log`，方便排查 writer 拿到了什么信息。
- 日志包含五段完整性检查，缺失段会标注并写入 problems，最终报告不会静默。
...
- `SKILL.md`：Step 1 与 Step 2 之间新增 Step 1.1 输入日志写入步骤。
...
- writer 输入日志新增"思考内容"字段，记录 writer 的完整思考逻辑，方便排查写作质量问题。
...
- `agents/writer.md`：输入日志增加思考内容段，修正重复的 section 10 编号。
